...
/* inet_pton(), inet_ntop() are libc functions which conform to
 * Linux include headers.
 * Linux address family constants are different than FreeBSD address family
 * constants, so need to use LINUX_AF_INET6 instead of AF_INET6.
 */
...
{
    struct in_addr v4;
    if (inet_pton(LINUX_AF_INET, ip_addr.c_str(), &v4)) {
